...
import { ArrowLeft, Clock, MapPin, ThumbsUp, ThumbsDown, Star, Send, MessageCircle, Trash2, Reply, ChevronDown, ChevronUp, Minus, Flag, Ban, MoreHorizontal } from 'lucide-react';
import { supabase } from '@/lib/database/supabase';
import { notFound } from 'next/navigation';
import { useLanguage } from '@/lib/contexts/LanguageContext';
import dynamic from 'next/dynamic';
import ReportModal from '@/components/map/ReportModal';
import SubmitProofButton from '@/components/map/SubmitProofButton';
...
export default function NeedDetailClient({ params }: { params: Promise<{ id: string }> }) {
    ...
    return (
        <div className="min-h-screen bg-gray-50 pb-20">
                ...
                <div className="max-w-3xl mx-auto flex items-center">
                    ...
                    <div className="relative">
                        <button
                            ...
                            className={`p-2 rounded-lg hover:bg-gray-100 transition ${dir === 'rtl' ? 'mr-auto' : 'ml-2'}`}
                        >
                            <MoreHorizontal className="w-5 h-5 text-gray-600" />
                        </button>
                        {showMenu && (
                            <div className={`absolute top-full ${dir === 'rtl' ? 'left-0' : 'right-0'} mt-1 bg-white rounded-xl shadow-lg border border-gray-200 py-1 z-50 min-w-[180px]`}>
                                <button
                                    onClick={() => { setShowMenu(false); setShowReportModal(true); }}
                                    className="w-full flex items-center gap-3 px-4 py-3 hover:bg-gray-50 transition text-left"
                                >
                                    <Flag className="w-4 h-4 text-amber-500" />
                                    <span className="text-sm text-gray-700">{dir === 'rtl' ? 'الإبلاغ عن المحتوى' : 'Report Content'}</span>
                                </button>
                                {need?.user_id && currentUser && need.user_id !== currentUser.id && (
                                    <button
                                        onClick={async () => {
                                            setShowMenu(false);
                                            if (!currentUser) return;
                                            if (confirm(dir === 'rtl' ? 'لن ترى محتوى هذا المستخدم بعد الآن. هل تريد المتابعة؟' : "You won't see this user's content anymore. Continue?")) {
                                                await supabase.from('blocked_users').insert({ blocker_id: currentUser.id, blocked_id: need.user_id });
                                    ...
                                    >
                                        ...
                                        <span className="text-sm text-red-600">{dir === 'rtl' ? 'حظر المستخدم' : 'Block User'}</span>
                                    </button>
                                )}
                            </div>
                        )}
            ...
            <main className="max-w-3xl mx-auto px-4 py-8">
                ...
                <div className="mt-6">
                    <SubmitProofButton needId={need.id} dir={dir} />
                </div>
